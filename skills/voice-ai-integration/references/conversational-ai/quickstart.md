...
If a named provider is outside the list (e.g. `openai`), treat as unsupported and resolve before continuing.
...
**EN:**
```text
[provider] is not in the supported list for this quickstart:
A. Switch to the supported default
B. Show me the supported list
```
...
**ZH:**
```text
如果你有默认 provider 的 key，可以按默认三段式继续：
A. 有 key，按默认继续
B. 没有 / 我要自定义 provider
```
...
**EN:**
```text
If you have the default provider keys, I can continue with the baseline:
A. I have the keys — continue with defaults
B. I want to customize providers
```
