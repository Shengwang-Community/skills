...
- 判定标准: 未逐个问 Q1/Q2/Q3，而是只展示未确认字段的选项和默认值
- 结果: ___
...
- 判定标准: 在一条回复里展示 credentials、App Certificate、ASR、ASR 语言、LLM、TTS 的选项，而不是拆成多轮
- 结果: ___

### I-05: 一次回复后产出结构化 spec

- 用户输入: "做一个 Web AI 语音助手，Python 服务端，凭证已就绪，App Certificate 没开，其他用默认"
- 期望行为: 直接归一化成 ConvoAI 结构化 spec 并继续
- 判定标准: 输出结构化 spec，不再额外要求单独确认
- 结果: ___
